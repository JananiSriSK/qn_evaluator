...
export default function ManageSubjects() {
  ...
  const handleDeleteSyllabus = async () => {
    if (!confirm("Delete syllabus? This will remove the syllabus file."))
      return;

    setLoading(true);
    try {
      await fetch(
        `http://localhost:8002/domains/${userId}/${selectedSubject}/syllabus`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleDeleteBook = async (bookName) => {
    ...
    try {
      await fetch(
        `http://localhost:8002/domains/${userId}/${selectedSubject}/books/${encodeURIComponent(bookName)}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleDeleteSubject = async () => {
    if (
      !confirm(
        `Delete entire subject: ${selectedSubject}? This cannot be undone.`,
      )
    )
      return;
  ...
  const handleRebuildIndex = async () => {
    if (
      !confirm(
        "Rebuild FAISS index? This will reprocess all books with current syllabus.",
      )
    )
      return;
    ...
    try {
      const response = await fetch(
        `http://localhost:8002/domains/${userId}/${selectedSubject}/rebuild`,
        {
          method: "POST",
  ...
  const handleViewSyllabus = async () => {
    try {
      const response = await fetch(
        `http://localhost:8002/domains/${userId}/${selectedSubject}/syllabus/view`,
      );
      const data = await response.json();
        ...
        <div>
          ...
          >
            ...
            <div style={{ marginBottom: "24px" }}>
              ...
              {Object.entries(syllabusData?.course_outcomes || {}).map(
                ([co, desc]) => (
                  <div
                    key={co}
                    style={{
          ...
          {!selectedSubject ? (
            ...
            >
              <p style={{ color: "#6b7280", fontSize: "16px" }}>
                No subjects available. Create one from Dashboard.
              </p>
            </div>
          ) : (
            <>
                    ...
                    <div style={{ display: "flex", gap: "12px" }}>
                      ...
                      >
                        ...
                        <input
                          type="file"
                          accept=".pdf,.txt"
                          onChange={handleReplaceSyllabus}
                          style={{ display: "none" }}
                          disabled={loading}
                        />
                      </label>
                      <a
                        href={`http://localhost:8002/domains/${userId}/${selectedSubject}/files/syllabus`}
                        target="_blank"
              ...
              >
                ...
                {subjectStatus?.books_uploaded &&
                subjectStatus.books.length > 0 ? (
                  <div>
                    <div style={{ marginBottom: "16px" }}>
                      ...
                      >
                        {subjectStatus.books.length} book(s) •{" "}
                        {subjectStatus.chunks_count} chunks indexed
                      </p>
                      ...
                      >
                        {subjectStatus.books.map((book, idx) => (
                          ...
                          >
                            <a
                              href={`http://localhost:8002/domains/${userId}/${selectedSubject}/files/books/${encodeURIComponent(book)}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              style={{
                                fontSize: "14px",
              ...
              >
                ...
                >
                  Delete this subject permanently. This will remove all
                  syllabus, books, and evaluation history.
                </p>
                <button
