...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Initialize history service with MongoDB if configured
        use_mongodb = os.getenv("USE_MONGODB", "false").lower() == "true"
        if use_mongodb:
            try:
                from services.history_repository import MongoHistoryRepository
                mongo_uri = os.getenv("MONGODB_URI", "mongodb://localhost:27017/")
                mongo_db = os.getenv("MONGODB_DB", "question_intelligence")
                logger.info(f"Using MongoDB for history storage: {mongo_db}")
                history_service = HistoryService(repository=MongoHistoryRepository(mongo_uri, mongo_db))
            except Exception as e:
                logger.warning(f"MongoDB initialization failed, falling back to file storage: {e}")
                history_service = HistoryService(base_path="data")
        else:
            logger.info("Using file-based history storage")
            history_service = HistoryService(base_path="data")
...
async def create_domain(request: CreateDomainRequest):
    ...
    try:
        domain_path = domain_manager.get_domain_path(request.user_id, request.domain_name)
        return {
...
async def list_domains(user_id: str):
    ...
    try:
        ...
        domains = [d.name for d in user_path.iterdir() if d.is_dir()]
...
async def upload_syllabus(
    ...
    """Upload and parse syllabus (PDF or TXT) - triggers full pipeline rebuild"""
    try:
        # Validate file type
        ...
        syllabus = domain_manager.save_syllabus(user_id, domain_name, temp_path)
        
        # Cleanup temp file
        temp_path.unlink()
        
        # Check if books exist - if yes, rebuild entire pipeline
        books_path = Path("data") / user_id / domain_name / "books"
        if books_path.exists() and list(books_path.glob("*.pdf")):
            logger.info("Syllabus updated - rebuilding FAISS index with new syllabus")
...
async def download_syllabus_file(user_id: str, domain_name: str):
    ...
    try:
        ...
        for ext in ['.pdf', '.txt']:
            files = list(domain_path.glob(f"syllabus{ext}"))
            if files:
...
async def upload_books(
    ...
    """Upload multiple books and build FAISS index"""
    try:
        ...
        temp_files = []
        for book in files:
            temp_path = Path(f"temp_{book.filename}")
            with open(temp_path, "wb") as f:
        ...
        # Add books and build index
        success = domain_manager.add_books(user_id, domain_name, temp_files, embedder)
        
        # Get metadata count
        _, metadata = domain_manager.load_index(user_id, domain_name)
...
async def delete_book(user_id: str, domain_name: str, book_name: str):
    """Delete a book and rebuild index"""
    try:
        books_path = Path("data") / user_id / domain_name / "books"
        book_file = books_path / book_name
...
async def get_domain_status(user_id: str, domain_name: str):
    """Get domain setup status with file details"""
    try:
        domain_path = Path("data") / user_id / domain_name
        if not domain_path.exists():
            raise HTTPException(status_code=404, detail={"error": "Domain not found"})
        ...
        # Get book names from metadata
        books = []
        if metadata:
            book_names = set(m.get("book_name") for m in metadata if m.get("book_name"))
            books = list(book_names)
        
        # Get syllabus filename
        syllabus_file = None
        ...
        return {
            "syllabus_uploaded": syllabus is not None,
            "syllabus_file": syllabus_file,
            "books_uploaded": index is not None,
            "books": books,
